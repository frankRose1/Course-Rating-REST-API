...
courseController.createReviewRefactor = (req, res, next) => {
  ...
  req.body.rating = typeof(req.body.rating) === 'number' ? req.body.rating : parseInt(req.body.rating);
  ...
  review
    ...
    .catch(err => {
      ...
      if (err.name === 'ValidationError'){
        const error = new Error('Please leave a number rating between 1 and 5.');
        error.status = 400;
        throw error;
      } else {
        throw err;
